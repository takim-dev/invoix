...
<div class="topbar">
    ...
    <div>
        ...
        <a href="/invoices/<?= $invoice['id'] ?>/edit" class="btn btn-warning btn-sm me-1"><i class="bi bi-pencil me-1"></i> Edit</a>
        <form action="/invoices/<?= $invoice['id'] ?>/delete" method="POST" class="d-inline" data-confirm="Delete this invoice?">
            <button class="btn btn-danger btn-sm"><i class="bi bi-trash me-1"></i> Delete</button>
        </form>
        <a href="/invoices" class="btn btn-outline-secondary btn-sm ms-1"><i class="bi bi-arrow-left me-1"></i> Back</a>
    </div>
</div>
        ...
        <div class="row">
            <div class="col-md-6">
                <h4 style="color:#c9a84c;margin-bottom:1.5rem;">
                    ...
                    <?php endif; ?>
                    <?= esc($invoice['company_name'] ?? 'Company') ?>
                </h4>
                ...
                <?php if ($invoice['company_tax_number']): ?><p class="text-muted" style="margin:0;">NPWP: <?= esc($invoice['company_tax_number']) ?></p><?php endif; ?>
            </div>
            <div class="col-md-6 text-md-end">
                <h5><?= esc($invoice['invoice_number']) ?></h5>
                <p class="text-muted" style="margin:0;">Date: <?= date('d M Y', strtotime($invoice['invoice_date'])) ?></p>
                <?php if ($invoice['due_date']): ?><p class="text-muted" style="margin:0;">Due: <?= date('d M Y', strtotime($invoice['due_date'])) ?></p><?php endif; ?>
    ...
    <div class="card-body p-0">
        <div class="table-responsive">
            <table class="table mb-0">
                <thead><tr><th>#</th><th>Description</th><th class="text-center">Qty</th><th class="text-end">Price</th><th class="text-end">Total</th></tr></thead>
                <tbody>
